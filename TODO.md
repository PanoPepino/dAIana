...
- [ ] Create a good, cheap!!! prompt to extract information on the company challenges and CRAFT a tailored sentence based on those chanllenges. 
...
- [ ] Create some sort of init. This should deploy the templates in a desired location, and ask the user for name, surname, phone, and things of the like to go and substitute things in the latex files (Perhaps create an overall contact.tex to subthings?). Then it should ask for the linkedin link, check and recommend 3/4 different career paths. If so, it will create 3/4 .csv with those names in the job_tracking folder. Then it should fix the specific instructiosn for the AI inputs and deploy those specific careers inside the latex files to easily manipulate when compiling.
