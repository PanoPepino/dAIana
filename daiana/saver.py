...
from daiana.utils import check_dir_exist, rewrite_filename


def save_job_in_csv(career_path: str,
                    job_position: str,
                    company_name: str,
                    location: str,
                    status: str,
                    job_link: str) -> Path:
    """
    This function is designed to eat several input and store them as columns in a given `career_path`.csv.

    Args:
        career_path (str): The career path (i.e. the .csv document) where things will be stored. If it does not yet exist, it will be created.
